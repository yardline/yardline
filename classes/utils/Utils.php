...
use Yardline\Utils\HTML;

//se GroundhoggSMS\Classes\SMS;
...
class Utils {
	...
	public function get_object( $id = 0, $by = 'ID', $object = 'contact', $get_from_cache = true ) {
		...
		$class = apply_filters( 'yardline/utils/get_object', $class, $object );
	...
	public function encrypt_decrypt( $string, $action = 'e' ) {
		...
		if ( ! Plugin::$instance->settings->get_option( 'yl_secret_key', false ) ) {
			update_option( 'yl_secret_key', bin2hex( openssl_random_pseudo_bytes( 32 ) ) );
		}

		if ( ! Plugin::$instance->settings->get_option( 'yl_secret_iv', false ) ) {
			update_option( 'yl_secret_iv', bin2hex( openssl_random_pseudo_bytes( 16 ) ) );
		}

		if ( in_array( $encrypt_method, openssl_get_cipher_methods() ) ) {

			$secret_key = Plugin::$instance->settings->get_option( 'yl_secret_key' );
			$secret_iv  = Plugin::$instance->settings->get_option( 'yl_secret_iv' );
