...
use Yardline\DB\Manager;
use Yardline\utils\Installer;
use Yardline\Plugin;
...
class Main_Installer extends Installer {
	...
	public function plugin_activated( $plugin ) {
		...
		if ( $plugin == plugin_basename( YARDLINE__FILE__ ) && ! is_white_labeled() ) {
			if ( Plugin::$instance->settings->is_option_enabled( 'yl_guided_setup_finished' ) ) {
				exit( wp_redirect( admin_url( 'admin.php?page=yardline' ) ) );
			} else {
				exit( wp_redirect( admin_url( 'admin.php?page=yl_guided_setup' ) ) );
			}
		}
	}
}