 ...
 *
 * @since       1.0
 * @author      Yardline thanks Adrian Tobey for the assist <info@groundhogg.io>
 * @copyright   Copyright (c) 2020, Forte Press Inc.
...
abstract class Installer {
	...
	public function __construct() {
		...
		add_action( 'yardline/admin/tools/install', [ $this, 'show_manual_install' ] ); // DO LAST
	...
	public function show_manual_install() {
		...
		echo html()->e( 'a', [
			...
			'href'  => add_query_arg( [
				...
				'manual_install_nonce' => wp_create_nonce( 'yl_manual_install' ),
			], $_SERVER['REQUEST_URI'] )
	...
	public function do_manual_install() {
		...
		if ( get_request_var( 'manual_install' ) !== $this->get_installer_name() || ! wp_verify_nonce( get_request_var( 'manual_install_nonce' ), 'yl_manual_install' ) || ! current_user_can( 'install_plugins' ) ) {
			return;
		}
	...
	/**
	 * When a new Blog is created in multisite, see if YL is network activated, and run the installer
	 *
	 * @since  2.5
	 *
	 * @param $blog \WP_Site
