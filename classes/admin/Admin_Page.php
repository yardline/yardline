...
abstract class Admin_Page extends Supports_Errors {
	...
	protected function wp_die_no_access() {
		if ( wp_doing_ajax() ) {
			return wp_send_json_error( __( "Invalid permissions.", 'yardline' ) );
		}

		return wp_die( __( "Invalid permissions.", 'yardline' ), 'No Access!' );
	}
	...
	public function process_action() {
		...
		$action_or_filter = "yardline/admin/{$this->get_slug()}/process/{$this->get_current_action()}";
		...
		set_transient( 'yardline_last_action', $this->get_current_action(), 30 );
