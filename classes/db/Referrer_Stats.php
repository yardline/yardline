 ...
 *
 */
class Referrer_Stats extends DB {
	...
	public function get_for_date_range( $start_date, $end_date ) {
		global $wpdb;
		$sql = $wpdb->prepare( "SELECT date, visitors, pageviews FROM {$wpdb->prefix}yl_referrer_stats s WHERE s.date >= %s AND s.date <= %s", array( $start_date, $end_date ) );
