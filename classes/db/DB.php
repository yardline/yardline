...
abstract class DB {
	...
	public function render_table_name() {
		 ...
		 * @param DB     $db
		 */
		$this->table_name = apply_filters( 'yardline/db/render_table_name', $table_name, $this );
	}
	...
	public function get( $row_id ) {
		...
		return apply_filters( 'yardline/db/get/' . $this->get_object_type(), $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $this->table_name WHERE $this->primary_key = %s LIMIT 1;", $row_id ) ) );
	}
	...
	public function get_column( $column, $row_id ) {
		...
		return apply_filters( 'yardline/db/get_column/' . $this->get_object_type(), $wpdb->get_var( $wpdb->prepare( "SELECT $column FROM $this->table_name WHERE $this->primary_key = %s LIMIT 1;", $row_id ) ) );
	}
	...
	public function insert( $data ) {
		...
		do_action( 'yardline/db/pre_insert/' . $this->get_object_type(), $data );
		...
		$data = apply_filters( 'yardline/db/pre_insert/' . $this->get_object_type(), $data, $column_formats );
		...
		do_action( 'yardline/db/post_insert/' . $this->get_object_type(), $wpdb_insert_id, $data );
	...
	public function update( $row_id = 0, $data = [], $where = [] ) {
		...
		do_action( 'yardline/db/pre_update/' . $this->get_object_type(), $where, $data );

		$data = apply_filters( 'yardline/db/pre_update/' . $this->get_object_type(), $data, $where );
		...
		do_action( 'yardline/db/post_update/' . $this->get_object_type(), $where );
	...
	public function mass_update( $data, $where ) {
		...
		do_action( 'yardline/db/pre_mass_update/' . $this->get_object_type(), $data );

		$data = apply_filters( 'yardline/db/mass_update/' . $this->get_object_type(), $data, $column_formats );

		if ( false === $wpdb->update( $this->table_name, $data, $where ) ) {
			return false;
		}

		do_action( 'yardline/db/post_mass_update/' . $this->get_object_type(), $data );
	...
	public function bulk_delete( $where = [] ) {
		...
		do_action( 'yardline/db/pre_bulk_delete/' . $this->get_object_type(), $where );

		$result = $wpdb->delete( $this->table_name, $where );

		do_action( 'yardline/db/post_bulk_delete/' . $this->get_object_type(), $where );
	...
	public function delete( $row_id = 0 ) {
		...
		do_action( 'yardline/db/pre_delete/' . $this->get_object_type(), $row_id );
		...
		do_action( 'yardline/db/post_delete/' . $this->get_object_type(), $row_id );
	...
	public function advanced_query( $query_vars = [], $from_cache = true ) {
		...
		$results = apply_filters( 'yardline/db/query/' . $this->get_object_type(), $results, $query_vars );
	...
	public function get_column_by( $column, $column_where, $column_value ) {
		...
		return apply_filters( 'yardline/db/get_column/' . $this->get_object_type(), $wpdb->get_var( $wpdb->prepare( "SELECT $column FROM $this->table_name WHERE $column_where = %s LIMIT 1;", $column_value ) ) );
	}
