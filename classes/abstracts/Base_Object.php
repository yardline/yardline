...
abstract class Base_Object extends Supports_Errors implements Serializable, ArrayAccess {
	...
	protected function setup_object( $object ) {
		...
		$object = apply_filters( "yardline/{$this->get_object_type()}/setup", $object );
	...
	public function update( $data = [] ) {
		 ...
		 */
		do_action( "yardline/{$this->get_object_type()}/pre_update", $this->get_id(), $data, $this, $old_data );
		...
		if ( $updated = $this->get_db()->update( $this->get_id(), $data, $this->get_identifier_key() ) ) {
			 ...
			 * @param mixed[] $old_data the current data
			 */
			do_action( "yardline/{$this->get_object_type()}/post_update", $this->get_id(), $data, $this, $old_data );
	...
	public function create( $data = [] ) {
		 ...
		 */
		do_action( "yardline/{$this->get_object_type()}/pre_create", $data, $this );
		...
		if ( $id = $this->get_db()->add( $data ) ) {
			 ...
			 * @param Base_Object $object the object class
			 */
			do_action( "yardline/{$this->get_object_type()}/post_create", $this->get_id(), $data, $this );
	...
	public function get_as_array() {
		 ...
		 */
		return apply_filters( "yardline/{$this->get_object_type()}/get_as_array", [
			'ID'   => $this->get_id(),
			'data' => $this->data
		] );
	}
